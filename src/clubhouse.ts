...
export function inviteePreamble(
  ...
  return (
    ...
    `${room}${place}\n` +
    `당신은 ${nameOf(speaker)} 본인으로서 이 대화에 자연스럽게 참여하세요:\n` +
    `- 사용자와 동료의 말에 직접 반응하세요. 동료가 이미 한 말을 반복하지 말고, 보태거나(다른 관점·검증·반론) 역할을 나눠 맡으세요.\n` +
    `- 작업이 필요하면 설명만 하지 말고 당신의 도구로 실제 파일/명령으로 처리하세요(위 작업 디렉터리 기준).\n` +
    `- 당신의 내부 절차(어떤 스킬을 쓰는지, 세션 설정·규칙 확인 등)는 대화에 적지 마세요 — 인사·의견·결과만 자연스럽게.\n` +
    `[사교 — 선택] 작업과 별개로 회고나 잡담이 떠오르면 <회고>…</회고> 또는 <잡담>…</잡담> 태그로 덧붙여도 ` +
    `됩니다(없으면 안 써도 됩니다). 동료 의견이 궁금하면 그 태그 안에서 이름을 부르세요(예: ` +
    `"${others[0] ?? "동료"}, 너는 어떻게 생각해?"). 태그 밖은 작업창, 태그 안은 Clubhouse 에만 보입니다.`
  );
}
